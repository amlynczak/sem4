...
List& List::insert(CharData c, Insertion position){
    Node *new_node = new Node{new CharData(c), nullptr};
    if(_head == nullptr){
        _head = _end = new_node;
    }else if(position == Begin){
        new_node->next = _head;
...
Fraction* List::find(Fraction frac)const{
    return &frac;
}
...
List List::copy_if(bool text)const{
    const List a;
    return a;
}
